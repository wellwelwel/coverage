...
- **The runtimes list is the single source of truth for E2E iteration.** It lives in the test utils directory; never hardcode runtimes in individual tests.
- **Snapshots are regenerated via `npm run build:snapshots`, never hand-edited.** Paths inside snapshots are normalized relative to the fixture root — if an absolute path leaks in, fix the normalization, not the snapshot.
