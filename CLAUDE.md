...
- **The runtimes list is the single source of truth for E2E iteration.** It lives in the test utils directory; never hardcode runtimes in individual tests. `runtimesFor` cross-filters `(platform × reporter)`: all platforms run Node, Bun and Deno (intersected with reporter support).
